...
SELECT * FROM customer ORDER BY customer.first_name DESC;
...
FROM film
WHERE film.title = 'fifty shades of Grey';
...
SELECT film.film_id, film.title, film.description, film.length
FROM film
WHERE film.title ILIKE 'fi%';

-- 10) Write a query which will find the 10 cheapest movies.
SELECT *
FROM film
ORDER BY film.rental_rate LIMIT 10;
...
SELECT *
FROM film f
LEFT JOIN inventory i ON i.film_i = f.film_id
